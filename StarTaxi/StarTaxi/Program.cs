...
namespace StarTaxi
{
   internal class Program
   {
      ...
      public static long channelOrder = -1001860678113;
      public static long channelDriver = -1001528750159;
      public static long channelSupport = -1001845851895;
      public static long channelConsole = -1001758234342;

      // dev
      //private static string token { get; set; } = "5669311078:AAHja_WrIe4OknG-rEWNVbHjVDbis7ZpNiA";
      //public static long channelOrder = -1001584630610;
      //public static long channelDriver = -1001811346806;
      //public static long channelSupport = -1001599434864;
      //public static long channelConsole = -1001823279865;
                     ...
                     if (msg.bot_message != "none") {
                        if (msg.bot_message == "WaitName")
                           ...
                           try {
                              ...
                              InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("🚕 Стандарт", "StandartTaxi") }, new[] { InlineKeyboardButton.WithCallbackData("💺 Детский", "ChildTaxi") }, new[] { InlineKeyboardButton.WithCallbackData("🐶 Перевозка животных", "AnimalTaxi") }, new[] { InlineKeyboardButton.WithCallbackData("⛔️ Отменить", "CancelTaxi") } });
                              try {
                                 await client.EditMessageReplyMarkupAsync(message.Chat.Id, message.MessageId - 1, replyMarkup: null);
                              } catch { }
                              if (message.Location != null) {
                                 ...
                                 await client.SendTextMessageAsync(message.Chat.Id, "*Вызов такси*\n\nВыберите тариф:", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: keyboard);
                              }
         ...
         try {
            ...
            if (data == "ChangeName") {
               ...
               try {
                  ...
                  InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Детское кресло водителя", "ChildTaxiDrive") }, new[] { InlineKeyboardButton.WithCallbackData("Своё детское кресло", "ChildTaxiMe") }, new[] { InlineKeyboardButton.WithCallbackData("⛔️ Отменить", "Cancel") } });
                  await client.SendTextMessageAsync(message.Chat.Id, "*Вызов такси*\n\nВыберите тип детского кресла", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: keyboard);
               } catch { }
            }
            else if (data == "ChildTaxiMe") {
               try {
                  await client.DeleteMessageAsync(message.Chat.Id, message.MessageId);
               } catch { }
               Connect.LoadOrder(orders);
               var order = orders.Find(x => x.id_client == message.Chat.Id.ToString() && x.status == "wait");
               Connect.Query("update `Order` set tarif = 'childme' where id = " + order.id + ";");
               CallTaxi(message, order, "childme");
            }
            else if (data == "ChildTaxiDrive") {
               try {
                  await client.DeleteMessageAsync(message.Chat.Id, message.MessageId);
               } catch { }
               Connect.LoadOrder(orders);
               var order = orders.Find(x => x.id_client == message.Chat.Id.ToString() && x.status == "wait");
               Connect.Query("update `Order` set tarif = 'childdrive' where id = " + order.id + ";");
               CallTaxi(message, order, "childdrive");
            }
               ...
               if (driver != null) {
                  ...
                  string[] medias = { driver.photo_face, driver.photo_auto };
                  Telegram.Bot.Types.IAlbumInputMedia[] mediaGroup = GetMedia(medias, request);
                  await client.SendMediaGroupAsync(channelDriver, mediaGroup);
                  await client.SendTextMessageAsync(channelDriver, "Выберите действие\n\n" + message.Chat.Id, replyMarkup: keyboard);
               ...
               InlineKeyboardMarkup keyboard = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData(list[0], "SelectCategoryStandart") }, new[] { InlineKeyboardButton.WithCallbackData(list[1], "SelectCategoryChild") }, new[] { InlineKeyboardButton.WithCallbackData(list[2], "SelectCategoryAnimal") }, new[] { InlineKeyboardButton.WithCallbackData("Начать смену", "StartWork") }, new[] { InlineKeyboardButton.WithCallbackData("Отменить", "Cancel") } });
               ...
               await client.SendTextMessageAsync(channelSupport, "№" + answer[^1].id_message + "\n" + request);
            }
               ...
               if (category == "standart") tarif = "Стандарт";
               else if (category == "childme") {
                  tarif = "Детский (своё кресло)";
                  category = "child";
               }
               else if (category == "childdrive") {
                  tarif = "Детский (кресло водителя)";
                  category = "child";
               }
               else if (category == "animal") tarif = "Перевозка животных";
                     ...
                     while (true) {
                        try {
                           ...
                           if (freeWorkers.Count > 0) {
                              if (!canceled.Contains(worker.id_driver) && worker.status == "free") {
               ...
               if (order != null) {
                  ...
                  if (driver != null) {
                     ...
                     if (driverInfo != null) {
                        ...
                        await client.SendTextMessageAsync(order.id_client, "🚖 *Водитель ожидает вас в месте назначения*", Telegram.Bot.Types.Enums.ParseMode.Markdown);
                     }
                  }
               }
            }
      ...
      private static async void UpdateData(object sender, UpdateEventArgs e)
      {
         try {
            ...
            if (update.ChannelPost != null) {
               if (update.ChannelPost.Chat.Id == channelSupport) { // support
               ...
               else if (update.ChannelPost.Chat.Id == channelConsole) { // console
                  try {
                     await client.DeleteMessageAsync(update.ChannelPost.Chat.Id, update.ChannelPost.MessageId);
                  } catch { }
               ...
               else if (update.ChannelPost.Chat.Id == channelOrder) { // orders
                  try {
                     await client.DeleteMessageAsync(update.ChannelPost.Chat.Id, update.ChannelPost.MessageId);
                  } catch { }
               }
               else if (update.ChannelPost.Chat.Id == channelDriver) { // drivers
                  try {
                     await client.DeleteMessageAsync(update.ChannelPost.Chat.Id, update.ChannelPost.MessageId);
                  } catch { }
               }
...
#pragma warning restore IDE1006
      ...
      public static RouteData GetWayTime(string start_longitude, string start_latitude, string end_longitude, string end_latitude)
      {
         try {
            ...
            int price = Convert.ToInt32((32 - Math.Round(map.routes[0].distance / 1000, 1) / 100) * Math.Round(map.routes[0].distance / 1000, 1));
      ...
      public async static void CallTaxi(Telegram.Bot.Types.Message message, Order order, string type)
      {
         ...
         if (order != null) {
            ...
            if (type == "standart") type = "Стандарт";
               ...
               if (type == "childme")
                  type = "Детский (своё кресло)";
               else if (type == "childdrive")
                  type = "Детский (кресло водителя)";
            }
      ...
      public static string[] GetCategory(string id)
      {
         try {
            ...
            string[] list = { "Стандарт", "Детский", "Перевозка животных" };
