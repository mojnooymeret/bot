                              ...
                              if (!canceled.Contains(driver.id) && driver.status == "free") {
                                 ...
                                 for (int j = 0; j < 25; j++) {
                                    ...
                                    if (orders.Find(x => x.id.ToString() == id && x.id_driver == driver.id) != null) { // accept order
                                       ...
                                       if (order != null) {
                                          try {
                                             driver = activeDrivers.Find(x => x.id == driver.id);
...
#pragma warning restore IDE1006
         ...
         try {
            if (operation == "delete") {
               ...
               if (activeDrivers.Count > 1) {
                  List<ActiveDriver> temp = new List<ActiveDriver>();
                  for (int i = 1; i <= activeDrivers.Count - 1; i++)
      ...
      public static async void CheckActive()
      {
         try {
            while (true) {
               if (activeDrivers != null) {
                  ...
                  for (int i = 0; i < activeDrivers.Count; i++) {
                     try {
                        driver = null;
                        driver = drivers.Find(x => x.id == activeDrivers[i].id);
                        if (driver != null) {
                           if (driver.status == "work")
                              activeDriver.Add(activeDrivers[i]);
                        }
                     } catch { }
                  }
