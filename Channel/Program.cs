   ...
   internal class Program
   {
      ...
      public static async Task GetChanels(Telegram.Bot.Types.Message message, bool write, bool load)
      {
         try {
            ...
            response += "\nДля добавления/удаления канала в/из парсинг(-а) введите /addchannel [id канала от] [id темы от] [id темы в] (номера можно вводить через пробел, например /addchannel 12 32 43)";
            if (write == true)
         ...
         try {
            ...
            for (int i = 0; i < chanels.Count; i++) {
               try {
                  ...
                  try {
                     Chat tchat = (Chat)chats.chats[chanels[i].id];
                     channel = tchat.migrated_to;
                  } catch {
                     try {
                        Channel tchannel = (Channel)chats.chats[chanels[i].id];
                        channel = tchannel;
                     } catch { }
                  }
                  Messages_ForumTopics get = null;
                  if (channel != null)
                     get = await wclient.Channels_GetForumTopicsByID(channel, Convert.ToInt32(chanels[i].topic.Split('|')[0]));
                  if (get != null) {
                     if (get.messages.Length < 1) return;
                     foreach (var msgBase in get.messages.Reverse()) {
                        if (msgBase is Message msg) {
                           Message message = (Message)msgBase;
                           Connect.LoadPost(posts);
                           var post = posts.Find(x => x.id.Split('|')[0] == chanels[i].id.ToString() && x.id.Split('|')[1] == message.ID.ToString());
                           if (post == null) {
                              if (message.media == null || message.media.ToString() == "TL.MessageMediaWebPage") {
                                 using (var http = new HttpClient()) {
                                    string url = $"https://api.telegram.org/bot{token}/sendMessage?chat_id={channelId}&message_thread_id={chanels[i].topic.Split('|')[1]}&text={message.message}";
                                 ...
                                 try {
                                    ...
                                    if (File.Exists(Path.GetFullPath(filename))) {
                                       ...
                                       string url = "https://api.telegram.org/bot" + token + "/sendPhoto?chat_id=" + channelId + "&message_thread_id=" + chanels[i].topic.Split('|')[1] + "&caption=" + message.message;
                                       using (var http = new HttpClient()) {
                                    ...
                                    try {
                                       ...
                                       if (File.Exists(Path.GetFullPath(filename))) {
                                          ...
                                          if (doc.mime_type.Contains("ogg")) {
                                             url = "https://api.telegram.org/bot" + token + "/sendVoice?chat_id=" + channelId + "&message_thread_id=" + chanels[i].topic.Split('|')[1] + "&caption=" + message.message;
                                             type = "voice";
                                          }
                                          else if (doc.mime_type.Contains("mpeg") || doc.mime_type.Contains("mp3")) {
                                             url = "https://api.telegram.org/bot" + token + "/sendAudio?chat_id=" + channelId + "&message_thread_id=" + chanels[i].topic.Split('|')[1] + "&caption=" + message.message;
                                             type = "audio";
                                          }
                                          else {
                                             url = "https://api.telegram.org/bot" + token + "/sendVideo?chat_id=" + channelId + "&message_thread_id=" + chanels[i].topic.Split('|')[1] + "&caption=" + message.message;
                                             type = "video";
                                          }
                                          using (var http = new HttpClient()) {
                                             using (var content = new MultipartFormDataContent("Upload----" + DateTime.Now.ToString(System.Globalization.CultureInfo.InvariantCulture))) {
